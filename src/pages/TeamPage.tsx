...
export default function TeamPage() {
  ...
  const profileAxes = ['BAT', 'BOWL', 'WK', 'RATING', 'VETS'];
  ...
  const pulse = [{ label: 'READINESS', value: `${Math.round((players.length / SQUAD_SIZE) * 100)}%`, percent: (players.length / SQUAD_SIZE) * 100 }, { label: 'BUDGET CONTROL', value: auction ? `${Math.round(100 - pursePercent)}%` : '—', percent: auction ? 100 - pursePercent : 0 }, { label: 'EXPERIENCE', value: players.length ? `${Math.round((vetCount / players.length) * 100)}%` : '—', percent: players.length ? (vetCount / players.length) * 100 : 0 }];
