...
export default function TeamPage() {
  ...
  const freeLeaderKeys = useMemo(() => new Set(players.filter((player) => ['owner', 'co_owner', 'captain'].includes(player.role)).map((player) => playerKey(player.name))), [players]);
  const results = useMemo(() => (team ? auction?.results.filter((result) => result.status === 'sold' && result.team_code === team.code && !freeLeaderKeys.has(playerKey(result.player_name))) ?? [] : []), [team, auction, freeLeaderKeys]);
        ...
        {!loaded ? <div className="teams-board-loading"><span /> Loading team…</div> : !team ? (
          ...
          <>
            ...
            <section className="team-detail-metrics" aria-label="Team analytics">
              ...
              <div><span>AVG BUY</span><b>{averageBuy ? formatCompact(averageBuy) : '—'}</b><small>{results.length} paid players</small></div>
